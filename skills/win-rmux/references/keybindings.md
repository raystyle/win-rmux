...
前缀键：`C-b`（`prefix C-b`，`prefix2` 未设置）。输入前缀后按下列键。
...
| --- | --- | --- |
| `c` | 新建窗口 | new-window |
| `d` | 分离客户端 | detach-client |
| `"` | 上下分割 | split-window |
| `%` | 左右分割 | split-window -h |
| `Space` | 下一布局 | next-layout |
| `z` | 缩放窗格 | resize-pane -Z |
| `x` | 杀窗格（确认） | kill-pane |
| `&` | 杀窗口（确认） | kill-window |
| `[` / `]` | 进复制模式 / 粘贴 | copy-mode / paste-buffer |
| `?` | 列出键位 | list-keys -N |
| `:` | 命令提示 | command-prompt |
| `s` / `w` | 选会话 / 选窗口 | choose-tree -Zs / -Zw |
| `p` / `n` | 上 / 下一窗口 | previous-window / next-window |
| `q` | 显示窗格编号 | display-panes |
...
- prefix 表：`rmux list-keys -T prefix`
- root 表（复制模式等）：`rmux list-keys`
- 列出带说明：`rmux list-keys -N`
...
## 完整 prefix 表（list-keys -T prefix）
